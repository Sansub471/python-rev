...
#Function to find a continuous sub-array which adds up to a given number.
class Solution:
    def subArraySum(self,arr, n, sum_given): 
        ...
        while i <= n:
            ...
            # If curr_sum exceeds the sum, then remove
            # the starting elements
            while curr_sum > sum_given and start < i-1:
                curr_sum -= arr[start] # slower than curr_sum = curr_sum + arr[start]
                start += 1
            ...
            if i < n:
                curr_sum += arr[i] # slower than curr_sum = curr_sum + arr[i]
            i += 1
